...
export default function ChatPage() {
  // Região a partir do localStorage
  ...
  // Se não houver região, redireciona
  useEffect(() => {
    if (!region) {
  ...
  // Tema (dark/light) via <html class="dark"> para Tailwind v4
  const [isDark, setIsDark] = useState(() => {
    try {
      const raw = localStorage.getItem("bepit_theme");
      return raw ? raw === "dark" : true; // padrão: escuro
    } catch {
      return true;
    }
  });
  ...
  const [messages, setMessages] = useState(() => {
    ...
    // Boas-vindas + lembrete para ver avisos
    ...
    return [
      { id: "welcome", role: "assistant", text: welcome, ts: Date.now() }
    ];
  });

  const [input, setInput] = useState("");

  // Sticky chips (sempre visíveis no topo do chat)
  ...
  // Referência para o fim da lista (autoscroll)
  ...
  return (
    <div className="min-h-dvh bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-50 flex flex-col">
      ...
      {/* Caixa de entrada — altura menor no mobile para não esconder */}
