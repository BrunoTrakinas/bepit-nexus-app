...
// - Embedding: Google "text-embedding-004" com outputDimensionality=768 (v1beta)
// - Coluna alvo: public.parceiros.embedding_768 (vector(768))
// - RPCs esperadas (devem usar embedding_768):
//     - public.parceiros_vector_search_v2(query_embedding vector, match_count int, filtro_cidade_id uuid, filtro_categoria text)
//     - public.parceiros_text_search_v2(q_ilike text, filtro_cidade_id uuid, filtro_categoria text, fetch_count int)
// ============================================================================
...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.warn("[RAG] SUPABASE_URL ou SUPABASE_SERVICE_ROLE_KEY ausente(s).");
}
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);

// Pesos de re-rank (ajuste fino conforme desejar)
const W_VECTOR = 0.85;
const W_TEXT = 0.15;
...
// ------------------------- Embedding (forçar 768d) --------------------------
// Usamos o endpoint v1beta text-embedding-004 + outputDimensionality: 768.
// Isso evita receber 3072 dimensões.
async function embedText768(text) {
  ...
  if (arr.length !== 768) {
    throw new Error(`Embedding dimension mismatch: got ${arr.length}, expected 768`);
  }
  return arr;
}

// ------------------------- INDEXAÇÃO (salvar 768d) --------------------------
/**
 * Indexa um parceiro:
 * - Se chunks vierem, concatena e embeda; senão, usa nome+descrição do banco.
 * - Salva em public.parceiros.embedding_768.
 */
export async function indexPartnerText(partnerId, chunks = []) {
  ...
  if (!baseText) {
    ...
    const combined = (nome + desc).trim();
    if (!combined) throw new Error("Sem conteúdo para indexar.");
    baseText = combined;
  }

  // 2) Gerar embedding 768d
  const embedding = await embedText768(baseText);

  // 3) Salvar na coluna embedding_768
  ...
  return {
    ok: true,
    partnerId,
    savedColumn: "embedding_768",
    dims: embedding.length,
    usedChunks: Array.isArray(chunks) ? chunks.length : 0,
  };
}
 ...
 * @param {string|null} params.cidade_id
...
export async function hybridSearch({ q, cidade_id, categoria, limit = 10, debug = false }) {
  ...
  // 1) Vetorial (usa RPC que consulta p.embedding_768)
  try {
    if (q && GEMINI_API_KEY) {
      meta.steps.vector_v2.tried = true;
      const qVec = await embedText768(q); // 768d
  ...
  if ((!textRows || textRows.length === 0) && q) {
    try {
      ...
      const qb = supabase
        .from("parceiros")
        .select("id, nome, descricao, cidade_id, categoria, rating")
        .or(`nome.ilike.%${q}%,descricao.ilike.%${q}%`)
        .limit(safeLimit * 3);

      const { data: rowsTbl, error: tblErr } = await qb;
...
// -------------------------- Stub compat (se algo antigo chamar) -------------
...
// === (DES)INDEXAÇÃO & ESQUECIMENTO =========================================

// Gera embedding e salva na coluna embedding_768; opcionalmente reativa (ativo=true)
export async function indexPartnerById(partnerId, { reactivate = false } = {}) {
  ...
  const baseText = [
    parceiro.nome || "",
    parceiro.categoria ? `Categoria: ${parceiro.categoria}` : "",
    parceiro.descricao || "",
  ]
    .filter(Boolean)
    .join("\n");
...
// Apaga embedding e marca inativo (fica invisível na busca)
export async function pausePartnerIndex(partnerId, { motivo } = {}) {
  const { error } = await supabase
    ...
    .update({
      ...
      embedding_768: null,
      bloqueado: true,
      bloqueado_motivo: motivo || "bloqueado pelo admin",
    })
    .eq("id", partnerId);
...
// Reativa e reindexa (gera embedding de novo)
...
// Esquece de vez (apaga linha)
// Se você usa soft delete, troque por: set ativo=false + embedding_768=null + bloqueado=true
 ...
 *  - cidade_id (opcional)
 *  - categoria (opcional)
 *  - onlyMissing: se true (default), só quem não tem embedding_768
 *  - limit: máximo de linhas buscadas no banco (default 500)
 */
export async function bulkIndexPartners({ cidade_id = null, categoria = null, onlyMissing = true, limit = 500 }) {
  // 1) Buscar candidatos no banco
  let qb = supabase
    .from("parceiros")
  ...
  // 2) Processar em série (poderia paralelizar com cuidado com rate limit)
  for (const pid of ids) {
    try {
      await indexPartnerText(pid, []); // usa nome+descrição se chunks = []
      ok++;
    } catch (e) {
      fail++;
  ...
  return {
    total_candidates: count ?? ids.length,
